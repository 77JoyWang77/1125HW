...
class PDFRAGSystem:
    ...
    def load_pdf(self, pdf_path):
        ...
        pages = loader.load()  # 每頁返回一個 Document 物件，可能有多頁
        return pages

    def load_multiple_pdfs(self, pdf_dir: str):
        """
        從目錄批量加載 PDF 文件
...
def main():
    ...
    documents = rag.load_multiple_pdfs("./pdfs")  # 嘗試方式 A：加載多個 PDF
    if documents:
        rag.create_vector_store(documents)
    elif not rag.load_existing_vectorstore():  # 嘗試方式 B：使用已存的向量資料庫
        print("\n❌ 請提供 PDF 文件")
